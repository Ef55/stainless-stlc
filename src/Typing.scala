...
object Typing {
  ...
  sealed trait TypeDeriv {
    ...
    def env: Environment = this match {
      ...
      case TAbsDeriv(e, _, _, _) => e
      case TAppDeriv(e, _, _, _) => e
    }
    ...
    def t: Type = this match {
      ...
      case TAbsDeriv(_, t, _, _) => t
      case TAppDeriv(_, t, _, _) => t
    }
    ...
    def term: Term = this match{
      ...
      case TAbsDeriv(_, _, term, _) => term
      case TAppDeriv(_, _, term, _) => term
    }
    ...
    def isValid: Boolean = {
      this match{
        ...
        case TAbsDeriv(env, t, tabs, btd) => btd.term == tabs.t && btd.isValid && btd.env == env && (t match{
          case UniversalType(b) => btd.t == b
          case _ => false
        })
        case TAppDeriv(env, t, tapp, btd) => btd.term == tapp.t && btd.isValid && btd.env == env && (btd.t match{
          case UniversalType(b) => t == absSubstitution(b, tapp.typ)
          case _ => false
        })
      }
    }
  ...
  case class VarDeriv(env: Environment, t: Type, term: Var) extends TypeDeriv
  ...
  case class TAbsDeriv(env: Environment, t: Type, term: TAbs, btd: TypeDeriv) extends TypeDeriv
  case class TAppDeriv(env: Environment, t: Type, term: TApp, btd: TypeDeriv) extends TypeDeriv


  def deriveType(env: Environment, t: Term): Option[TypeDeriv] = {
    t match {
      case v@Var(k) => if (k < env.size) Some(VarDeriv(env, env(k), v)) else None()
      ...
      case tabs@TAbs(t) => {
        deriveType(env, t) match{
          case Some(btd) => Some(TAbsDeriv(env, UniversalType(btd.t), tabs, btd))
          case None() => None()
        }
      }
      ...
      case tapp@TApp(t, typ) => {
        deriveType(env, t) match{
          case Some(btd) => {
            btd.t match{
              case UniversalType(b) => Some(TAppDeriv(env, absSubstitution(b, typ), tapp, btd))
              case _ => None()
            }
...
//   TypeOf
  ...
  def typeOfCompleteness(td: TypeDeriv): Unit ={
    require(td.isValid)
    deriveTypeCompleteness(td)
